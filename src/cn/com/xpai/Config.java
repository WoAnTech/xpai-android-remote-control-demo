...
public class Config {
	...
	static void load(Activity a) {
		...
		serviceKey = sp.getString("service_key", serviceKey);
		bufferSize = sp.getInt("buffer_size", bufferSize);
	}
	...
	static void save() {
		...
		editor.putString("service_key", serviceKey);
		editor.putInt("buffer_size", bufferSize);
		editor.commit();
	}
